...
private[sql] case class RTreeIndexRelation(
  ...
  private[sql] def buildIndex() : Unit = {
    val dataRDD = child.execute().map(row => {
//      val x = BindReferences
//        .bindReference(columnKeys.head, child.output)
//        .eval(row)
//        .asInstanceOf[Number].doubleValue
//      val y = BindReferences
...
//    dataRDD.collect().foreach(println)
    val (partitionedRDD, mbr_bounds) = strRangePartition(dataRDD, columnKeys.length, numShufflePartitions, sampleRate)
    val partitionedSize = partitionedRDD.mapPartitions{iter => new Iterator[Int] {
    ...
    //indexed.setName(tableName.map(n => s"RTree Indexed table $n").getOrElse(child.toString))
    indexed.setName(tableName.map(n => s"$n $indexName").getOrElse(child.toString))
    _indexedRDD = indexed
  }
