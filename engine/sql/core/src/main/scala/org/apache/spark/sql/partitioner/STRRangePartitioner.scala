...
object strRangePartition extends DataPartition{
  ...
  def apply(originRDD: RDD[(Point, Row)], dimension: Int, est_partition: Int, sampleRate: Double)
  : (RDD[(Point, Row)], Array[(MBR, Int)]) = {
    ...
    val part = new STRRangePartitioner(est_partition, sampleRate, dimension, rdd)
...
class STRRangePartitioner(est_partition: Int, sampleRate: Double, dimension: Int, rdd: RDD[_ <: Product2[Point, Any]]) extends Partitioner {
  def numPartitions = partitions

  case class Bounds(min: Array[Double], max: Array[Double])

  val (mbrBound, partitions) = {
    val data_bounds = rdd.aggregate[Bounds](null)((bound, data) => {
      if (bound == null) {
