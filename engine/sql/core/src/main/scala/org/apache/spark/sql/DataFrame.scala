...
import org.apache.spark.sql.catalyst.plans.{DistanceJoin, KNNJoin, JoinType, Inner}
import org.apache.spark.sql.catalyst.plans.logical._
import org.apache.spark.sql.execution.{EvaluatePython, ExplainCommand, LogicalRDD}
import org.apache.spark.sql.jdbc.JDBCWriteDetails
import org.apache.spark.sql.json.JsonRDD
import org.apache.spark.sql.types.{IntegerType, DoubleType, NumericType, StructType}
...
class DataFrame private[sql](
  ...
  def range(keys: Array[String], point1: Array[Double], point2: Array[Double]): DataFrame = {
    ...
    var boundL : Array[Literal] = Array()
    for (i <- 0 to point1.length - 1)
      boundL(i) = new Literal(point1(i), DoubleType)
    var boundR : Array[Literal] = Array()
    for (i <- 0 to point2.length - 1)
      boundR(i) = new Literal(point2(i), DoubleType)
    Filter(InRange(attrs, boundL, boundR), logicalPlan)
  }
  ...
  def circleRange(keys: Array[String], point: Array[Double], r: Double): DataFrame = {
    val attrs = getAttributes(keys)
    attrs.foreach(attr => assert(attr != null, "column not found"))
    var target : Array[Literal] = Array()
    for (i <- 0 to point.length - 1)
      target(i) = new Literal(point(i), DoubleType)
    Filter(InCircleRange(attrs, target, new Literal(r, DoubleType)), logicalPlan)
  }
  ...
  def knn(keys: Array[String], point: Array[Double], k: Int): DataFrame = {
    ...
    for (i <- 0 to point.length - 1)
      target(i) = new Literal(point(i), DoubleType)
    Filter(InKNN(attrs, target, new Literal(k, IntegerType)), logicalPlan)
  }

  /**
   * Spatial operation DistanceJoin
   */
  def distanceJoin(right: DataFrame, leftKeys: Array[String], rightKeys: Array[String], r: Double) : DataFrame = {
    ...
    Join(this.logicalPlan, right.logicalPlan, DistanceJoin,
      Some(InCircleRange(rightAttrs, leftAttrs, new Literal(r, DoubleType))))
  }
  ...
  def knnJoin(right: DataFrame, leftKeys: Array[String], rightKeys: Array[String], k : Int) : DataFrame = {
    ...
    Join(this.logicalPlan, right.logicalPlan, KNNJoin,
      Some(InKNN(rightAttrs, leftAttrs, new Literal(k, IntegerType))))
  }

  /**
   * (Scala-specific) Returns a new [[DataFrame]] where each row has been expanded to zero or more
   * rows by the provided function.  This is similar to a `LATERAL VIEW` in HiveQL. The columns of
   * the input row are implici  tly joined with each row that is output by the function.
   *
  ...
  def deindex(): this.type = deindex(false)
  def deindexByName(indexName : String) : this.type = {
    sqlContext.indexManager.tryDropIndexByNameQuery(this, indexName, false)
    this
  }
